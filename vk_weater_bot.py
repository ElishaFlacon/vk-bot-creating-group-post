...
class weath():
    ...
    region = mgr.weather_at_place('your region')
...
def image():
    ...
    idraw.text((40, 20), f"Погода: {weath.tr_weater_detail}",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 100), f"Температура: {weath.main_temp}°C",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 140), f"По ощущениям: {weath.feel_temp}°C",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 200), f"Ветер: {weath.speed_wind}м/с, {weath.wind_direction}",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 260), f"Давление: {weath.press} мм рт. ст.",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 320), f"Влажность: {weath.main_humidity}%",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 380), f"Облачность: {weath.cloud}%",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 440), f"Видимость: {weath.visibility}км",
               font=headline, fill=(0, 0, 0, 255))
...
def vk_bot():
    ...
    photo = ("image.png")
    ready_photo = upload.photo_wall(photo)
