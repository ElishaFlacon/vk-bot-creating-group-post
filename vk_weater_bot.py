...
def image():
    ...
    idraw.text((40, 20), f"Погода: {tr_weater_detail}",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 100), f"Температура: {main_temp}°C",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 140), f"По ощущениям: {feel_temp}°C",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 200), f"Ветер: {speed_wind}м/с, {wind_direction}",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 260), f"Давление: {press} мм рт. ст.",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 320), f"Влажность: {main_humidity}%",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 380), f"Облачность: {cloud}%",
               font=headline, fill=(0, 0, 0, 255))
    idraw.text((40, 440), f"Видимость: {visibility}км",
               font=headline, fill=(0, 0, 0, 255))
